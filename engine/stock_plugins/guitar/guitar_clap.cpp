// Public CLAP adapter for OneBeat Guitar. Sound generation lives in
// guitar_engine.* so the instrument can be read and tested without CLAP details.
...
GuitarPlugin& self(const clap_plugin_t* plugin) {
  return *static_cast<GuitarPlugin*>(plugin->plugin_data);
}

void handleEvent(GuitarPlugin& guitar, const clap_event_header_t* header) {
  ...
  if (header->type == CLAP_EVENT_NOTE_ON) {
    ...
    guitar.engine.noteOn(note->note_id, note->channel, note->key, note->velocity);
  } else if (header->type == CLAP_EVENT_NOTE_OFF || header->type == CLAP_EVENT_NOTE_CHOKE) {
    const auto* note = reinterpret_cast<const clap_event_note_t*>(header);
    guitar.engine.noteOff(note->note_id, note->channel, note->key,
                          header->type == CLAP_EVENT_NOTE_CHOKE);
  } else if (header->type == CLAP_EVENT_PARAM_VALUE) {
    const auto* event = reinterpret_cast<const clap_event_param_value_t*>(header);
    guitar.engine.setParameter(event->param_id, event->value);
    ...
    if (status == 0x90U && velocity > 0) {
      guitar.engine.noteOn(-1, channel, key, static_cast<double>(velocity) / 127.0);
    } else if (status == 0x80U || (status == 0x90U && velocity == 0)) {
      guitar.engine.noteOff(-1, channel, key, false);
    }
  }
}
...
clap_process_status pluginProcess(const clap_plugin_t* plugin, const clap_process_t* process) {
  GuitarPlugin& guitar = self(plugin);
  ...
  for (uint32_t index = 0; index < event_count; ++index) {
    ...
    if (event_time > cursor) {
      guitar.engine.render(output.data32, output.channel_count, cursor, event_time - cursor);
      cursor = event_time;
    }
    handleEvent(guitar, event);
  }
  if (cursor < process->frames_count) {
    guitar.engine.render(output.data32, output.channel_count, cursor,
...
bool paramsValueToText(const clap_plugin_t*, clap_id id, double value, char* display,
  ...
  switch (id) {
    case onebeat::stock::guitar::ParamPreset: {
      const uint32_t idx = std::clamp(
          static_cast<uint32_t>(clamped * static_cast<double>(onebeat::stock::guitar::PresetCount)),
          0U, static_cast<uint32_t>(onebeat::stock::guitar::PresetCount - 1));
      std::strncpy(display, onebeat::stock::guitar::presetName(idx), size - 1);
      display[size - 1] = '\0';
      return true;
    }
    case onebeat::stock::guitar::ParamDecay: {
      const double sec = 0.2 + clamped * clamped * 8.0;
      std::snprintf(display, size, "%.2f s", sec);
      return true;
    }
    case onebeat::stock::guitar::ParamRelease: {
      const double sec = 0.02 + clamped * clamped * 3.0;
      std::snprintf(display, size, "%.2f s", sec);
      return true;
    }
    case onebeat::stock::guitar::ParamModRate: {
      const double hz = 0.2 + clamped * 4.0;
      std::snprintf(display, size, "%.1f Hz", hz);
      return true;
    }
    case onebeat::stock::guitar::ParamPitch: {
      const double st = (clamped - 0.5) * 24.0;
      if (std::abs(st) < 0.05) {
...
bool noteGet(const clap_plugin_t*, uint32_t index, bool input, clap_note_port_info_t* info) {
  ...
  *info = {};
  ...
  std::strncpy(info->name, "Guitar", sizeof(info->name) - 1);
  return true;
}
...
struct SavedStateHeader {
  uint32_t magic = 0x4f424754U;  // 'OBGT'
  uint32_t version = 1;
};
...
bool stateSave(const clap_plugin_t* plugin, const clap_ostream_t* stream) {
  ...
  const auto& guitar = self(plugin);
  for (const auto& spec : ParameterSpecs) {
    const double val = guitar.engine.parameter(spec.id);
    if (stream->write(stream, &val, sizeof(val)) != static_cast<int64_t>(sizeof(val))) {
      return false;
    }
  }
  return true;
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream) {
  ...
  if (stream->read(stream, &header, sizeof(header)) != static_cast<int64_t>(sizeof(header)) ||
      header.magic != 0x4f424754U) {
    return false;
  }
  auto& guitar = self(plugin);
  for (const auto& spec : ParameterSpecs) {
    double val = 0.0;
    if (stream->read(stream, &val, sizeof(val)) != static_cast<int64_t>(sizeof(val))) {
      return false;
    }
    guitar.engine.setParameter(spec.id, val);
  }
  return true;
}
...
const char* Features[]{CLAP_PLUGIN_FEATURE_INSTRUMENT, CLAP_PLUGIN_FEATURE_SYNTHESIZER, "guitar",
                       CLAP_PLUGIN_FEATURE_STEREO, nullptr};
const clap_plugin_descriptor_t Descriptor{
    CLAP_VERSION,
    "dev.onebeat.stock.guitar",
    "OneBeat Guitar",
    "OneBeat",
    ...
    "",
    "1.0.0",
    "A responsive physical-modeled stock guitar with acoustic and electric models.",
    Features,
};
...
const clap_plugin_t* factoryCreate(const clap_plugin_factory_t*, const clap_host_t*,
  ...
  auto* guitar = new (std::nothrow) GuitarPlugin;
  if (guitar == nullptr) {
    return nullptr;
  }
  guitar->plugin.desc = &Descriptor;
  guitar->plugin.plugin_data = guitar;
  guitar->plugin.init = pluginInit;
  guitar->plugin.destroy = pluginDestroy;
  guitar->plugin.activate = pluginActivate;
  guitar->plugin.deactivate = pluginDeactivate;
  guitar->plugin.start_processing = pluginStart;
  guitar->plugin.stop_processing = pluginStop;
  guitar->plugin.reset = pluginReset;
  guitar->plugin.process = pluginProcess;
  guitar->plugin.get_extension = pluginExtension;
  guitar->plugin.on_main_thread = pluginMainThread;
