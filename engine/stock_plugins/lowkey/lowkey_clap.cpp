...
struct LowkeyPlugin { clap_plugin_t plugin{}; GuitarEngine engine; };
LowkeyPlugin& self(const clap_plugin_t* p) { return *static_cast<LowkeyPlugin*>(p->plugin_data); }
...
void handleEvent(LowkeyPlugin& bass, const clap_event_header_t* header) {
  ...
  if (header->type == CLAP_EVENT_NOTE_ON) {
    ...
    const auto* note = reinterpret_cast<const clap_event_note_t*>(header);
    bass.engine.noteOff(note->note_id, note->channel, std::max(0, note->key - 12), header->type == CLAP_EVENT_NOTE_CHOKE);
...
bool activate(const clap_plugin_t* p, double rate, uint32_t, uint32_t) { self(p).engine.setSampleRate(rate); return true; }
void deactivate(const clap_plugin_t*) {}
bool start(const clap_plugin_t*) { return true; }
void stop(const clap_plugin_t*) {}
void reset(const clap_plugin_t* p) { self(p).engine.reset(); }

clap_process_status process(const clap_plugin_t* p, const clap_process_t* process_info) {
  LowkeyPlugin& bass = self(p);
  if (process_info->audio_outputs_count == 0 || process_info->audio_outputs[0].data32 == nullptr) return CLAP_PROCESS_CONTINUE;
  ...
  uint32_t cursor = 0;
  const uint32_t count = process_info->in_events == nullptr ? 0U : process_info->in_events->size(process_info->in_events);
  for (uint32_t i = 0; i < count; ++i) {
    ...
    if (time > cursor) { bass.engine.render(output.data32, output.channel_count, cursor, time - cursor); cursor = time; }
    handleEvent(bass, event);
  }
  if (cursor < process_info->frames_count) bass.engine.render(output.data32, output.channel_count, cursor, process_info->frames_count - cursor);
  return CLAP_PROCESS_CONTINUE;
}

uint32_t paramsCount(const clap_plugin_t*) { return static_cast<uint32_t>(ParameterSpecs.size()); }
bool paramsInfo(const clap_plugin_t*, uint32_t index, clap_param_info_t* info) {
  ...
  *info = {};
  ...
  info->min_value = 0.0; info->max_value = 1.0; info->default_value = ParameterSpecs[index].default_value;
  return true;
}
bool paramsValue(const clap_plugin_t* p, clap_id id, double* value) {
  ...
  *value = self(p).engine.parameter(id); return true;
}
bool paramsText(const clap_plugin_t*, clap_id id, double value, char* display, uint32_t size) {
  ...
  if (id == onebeat::stock::guitar::ParamPreset) {
    static constexpr const char* names[] = {"Deep Pocket", "Tape Sub", "Picked Modern", "Neon Fretless", "Grit Room", "Synth Bass", "Wide Chorus", "808 Lowkey", "Moog Pressure", "Rubber Band", "Dub Siren", "Dark Cinema", "Slap Snap", "Acid Low End", "Clean Fingerstyle", "Submarine"};
    const size_t index = std::min<size_t>(static_cast<size_t>(clampParameter(value) * 16.0), 15);
    std::strncpy(display, names[index], size - 1); display[size - 1] = '\0'; return true;
  }
  std::snprintf(display, size, "%d %%", static_cast<int>(clampParameter(value) * 100.0)); return true;
}
bool paramsFromText(const clap_plugin_t*, clap_id id, const char* text, double* value) {
  ...
  *value = clampParameter(std::strtod(text, nullptr) / 100.0); return true;
}
void paramsFlush(const clap_plugin_t* p, const clap_input_events_t* input, const clap_output_events_t*) {
  if (input == nullptr) return;
  for (uint32_t i = 0; i < input->size(input); ++i) handleEvent(self(p), input->get(input, i));
}
const clap_plugin_params_t Params{paramsCount, paramsInfo, paramsValue, paramsText, paramsFromText, paramsFlush};
...
bool audioInfo(const clap_plugin_t*, uint32_t index, bool input, clap_audio_port_info_t* info) {
  ...
  *info = {}; info->id = 0; std::strncpy(info->name, "Main Output", sizeof(info->name) - 1); info->flags = CLAP_AUDIO_PORT_IS_MAIN; info->channel_count = 2; info->port_type = CLAP_PORT_STEREO; info->in_place_pair = CLAP_INVALID_ID; return true;
}
...
bool noteInfo(const clap_plugin_t*, uint32_t index, bool input, clap_note_port_info_t* info) {
  if (!input || index != 0 || info == nullptr) return false;
  *info = {}; info->id = 0; info->supported_dialects = CLAP_NOTE_DIALECT_CLAP | CLAP_NOTE_DIALECT_MIDI; info->preferred_dialect = CLAP_NOTE_DIALECT_CLAP; std::strncpy(info->name, "Lowkey Bass", sizeof(info->name) - 1); return true;
}
...
const char* Features[]{CLAP_PLUGIN_FEATURE_INSTRUMENT, CLAP_PLUGIN_FEATURE_SYNTHESIZER, "bass", CLAP_PLUGIN_FEATURE_STEREO, nullptr};
const clap_plugin_descriptor_t Descriptor{CLAP_VERSION, "dev.onebeat.stock.lowkey", "Lowkey", "OneBeat", "https://github.com/spix3l/onebeat", "", "", "0.2.0", "A deep, playable stock bass instrument.", Features};
uint32_t factoryCount(const clap_plugin_factory_t*) { return 1; }
const clap_plugin_descriptor_t* factoryDescriptor(const clap_plugin_factory_t*, uint32_t index) { return index == 0 ? &Descriptor : nullptr; }
const clap_plugin_t* factoryCreate(const clap_plugin_factory_t*, const clap_host_t*, const char* id) {
  ...
  auto* bass = new (std::nothrow) LowkeyPlugin; if (bass == nullptr) return nullptr;
  bass->plugin.desc = &Descriptor; bass->plugin.plugin_data = bass; bass->plugin.init = init; bass->plugin.destroy = destroy; bass->plugin.activate = activate; bass->plugin.deactivate = deactivate; bass->plugin.start_processing = start; bass->plugin.stop_processing = stop; bass->plugin.reset = reset; bass->plugin.process = process; bass->plugin.get_extension = extension; bass->plugin.on_main_thread = [](const clap_plugin_t*) {};
  return &bass->plugin;
}
const clap_plugin_factory_t Factory{factoryCount, factoryDescriptor, factoryCreate};
bool entryInit(const char*) { return true; }
void entryDeinit() {}
const void* entryFactory(const char* id) { return id != nullptr && std::strcmp(id, CLAP_PLUGIN_FACTORY_ID) == 0 ? &Factory : nullptr; }
}
extern "C" {
