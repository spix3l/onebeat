...
//
// Both goldens are the whole window, because both mockups are about something
// sitting *over* the workspace: the golden has to show what it is over, or it
// is a picture of a popover rather than a picture of the screen.
...
void main() {
  ...
  testWidgets('the layouts menu and the detached window render as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
      UiFrame(
        ...
        overlay: const Stack(
          children: <Widget>[
            // The menu is anchored under its pill; the window floats over the
            // right of the workspace, as the mockup shows both at once.
            Positioned(left: 92, top: 134, child: LayoutsMenu(vm: demoLayouts)),
            Positioned(
              left: 903,
              top: 104,
    ...
    await expectLater(
      ...
      uiGolden('workspace_window'),
    );
  });

  testWidgets('the mid-drag dock state renders as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
      const UiFrame(
        ...
        content: Stack(
          children: <Widget>[
            ...
            Positioned.fill(
              // The drag layer covers the content area only: the chrome does
              // not accept a drop, so it does not light up.
