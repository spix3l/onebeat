// Mixer strips (UI-B-09): a board per variant, plus the callbacks and the
// painter contract the goldens cannot show.
...
void main() {
  ...
  testWidgets('the docked meter strips render as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
    ...
    await expectLater(
      find.byKey(const Key('meters')),
      uiGolden('mixer_meter_strips'),
    );
  });

  testWidgets('the detached fader strips render as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
    ...
    await tester.pumpAndSettle();
    await expectLater(
      find.byKey(const Key('faders')),
      uiGolden('mixer_fader_strips'),
    );
  });
