// The extension manager's three screens (UI-C-11), plus the callbacks and the
// copy the goldens cannot assert on their own.
...
/// it. The offsets are the anchor's position in the frame, which UI-C-01 will
/// compute; here they are what the PNG measures.
...
void main() {
  ...
  testWidgets('the extension manager renders as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
    ...
    await expectLater(find.byKey(const Key('screen')), uiGolden('ext_manager'));
  });

  testWidgets('the empty state renders as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
    ...
    await tester.pumpAndSettle();
    await expectLater(find.byKey(const Key('screen')), uiGolden('ext_empty'));
  });

  testWidgets('a docked extension panel renders as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
    ...
    await tester.pumpAndSettle();
    await expectLater(find.byKey(const Key('screen')), uiGolden('ext_panel'));
  });
