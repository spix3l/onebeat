// Channel rack lane + chrome (UI-B-05): a board of the four lane states, a
// board of the toolbar/header/footer stack, and the callbacks the goldens
// cannot show.
...
void main() {
  ...
  testWidgets('the four lane states render as the golden', (
    WidgetTester tester,
  ) async {
    ...
    await pumpUi(
      ...
      size: Size(demoRackRowWidth, tokens.size.rackLaneHeight * 4),
    );
    await tester.pumpAndSettle();
    await expectLater(find.byKey(const Key('rows')), uiGolden('rack_row'));
  });

  testWidgets('toolbar, column captions and footer render as the golden', (
    WidgetTester tester,
  ) async {
    await pumpUi(
      tester,
      ...
      size: const Size(demoRackRowWidth, 120),
    );
    await tester.pumpAndSettle();
    await expectLater(
      find.byKey(const Key('chrome')),
      uiGolden('rack_toolbar'),
    );
  });
